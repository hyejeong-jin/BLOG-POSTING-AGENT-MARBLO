...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="meta">
                <p>?‘ì„±?? {post.created_at.strftime('%Y-%m-%d %H:%M')}</p>
                <p>ì¹´í…Œê³ ë¦¬: {post.category or 'General'}</p>
                {f'<p>?œê·¸: {", ".join(post.tags)}</p>' if post.tags else ''}
            </div>
        </div>
...
?‘ì„±?? {post.created_at.strftime('%Y-%m-%d %H:%M')}
ì¹´í…Œê³ ë¦¬: {post.category or 'General'}
{f'?œê·¸: {", ".join(post.tags)}' if post.tags else ''}
